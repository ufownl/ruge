...
namespace RUGE
{
	...
	STDMETHODIMP CAudioImpl::Initialize()
	{
		...
		if (fr!=FMOD_OK)
		{
			fprintf(stderr, "Can't create FMOD interface.\n");
			return -fr;
		}
		fr=m_pFmod->init(m_nMaxChannels, FMOD_INIT_NORMAL, NULL);
		if (fr!=FMOD_OK)
		{
			fprintf(stderr, "Can't init FMOD.\n");
			return -fr;
		}
		...
		m_pFmod->getDriver(&nDriver);
		m_pFmod->getDriverInfo(nDriver, szBuf, 256, NULL);
		fprintf(stderr, "Sound Device: %s\nMax Channels: %d\n\n", szBuf, m_nMaxChannels);
		return S_OK;
	}
