...
namespace RUGE
{
	...
	BOOL CGUI::ProcessCtrl(CControl *pCtrl)
	{
		...
		if (m_pApp->Input_KeyDown(VK_LBUTTON))
		{
			...
			bResult=bResult || pCtrl->MouseLBtn(TRUE, m_fCurX-pCtrl->m_Rect.x1, m_fCurY-pCtrl->m_Rect.y1);
		}
		if (m_pApp->Input_KeyDown(VK_RBUTTON))
		{
			m_pCtrlLock=pCtrl;
			SetFocus(pCtrl->m_nID);
			bResult=bResult || pCtrl->MouseRBtn(TRUE, m_fCurX-pCtrl->m_Rect.x1, m_fCurY-pCtrl->m_Rect.y1);
		}
		if (m_pApp->Input_KeyUp(VK_LBUTTON)) bResult=bResult || pCtrl->MouseLBtn(FALSE, m_fCurX-pCtrl->m_Rect.x1, m_fCurY-pCtrl->m_Rect.y1);
		if (m_pApp->Input_KeyUp(VK_RBUTTON)) bResult=bResult || pCtrl->MouseRBtn(FALSE, m_fCurX-pCtrl->m_Rect.x1, m_fCurY-pCtrl->m_Rect.y1);
