...
{
	HRESULT hr;
	RUGE::CTimerClassFactory *pFactory;

	if (!InlineIsEqualGUID(rclsid, __uuidof(RUGE::CTimerImpl))) return CLASS_E_CLASSNOTAVAILABLE;
	if (IsBadWritePtr(ppv, sizeof(void*))) return E_POINTER;
	*ppv=NULL;
	pFactory=new RUGE::CTimerClassFactory;
	pFactory->AddRef();
...
STDAPI DllRegisterServer()
{
	...
	TCHAR szClassDescription[]=_T("Timer class");
	TCHAR szThreadingModel[]=_T("Apartment");

	__try
	{
		lRet=RegCreateKeyEx(HKEY_CLASSES_ROOT, _T("CLSID\\{2A892422-4CDC-4583-8CD9-2305ED2616DB}"),
...
{
	RegDeleteKey(HKEY_CLASSES_ROOT, _T("CLSID\\{2A892422-4CDC-4583-8CD9-2305ED2616DB}\\InProcServer32"));
	RegDeleteKey(HKEY_CLASSES_ROOT, _T("CLSID\\{2A892422-4CDC-4583-8CD9-2305ED2616DB}"));
	return S_OK;
}