...
{
	HRESULT hr;
	RUGE::CRandomClassFactory *pFactory;

	if (!InlineIsEqualGUID(rclsid, __uuidof(RUGE::CRandomImpl))) return CLASS_E_CLASSNOTAVAILABLE;
	if (IsBadWritePtr(ppv, sizeof(void*))) return E_POINTER;
	*ppv=NULL;
	pFactory=new RUGE::CRandomClassFactory;
	pFactory->AddRef();
...
STDAPI DllRegisterServer()
{
	...
	TCHAR szClassDescription[]=_T("Random class");
	TCHAR szThreadingModel[]=_T("Apartment");

	__try
	{
		lRet=RegCreateKeyEx(HKEY_CLASSES_ROOT, _T("CLSID\\{8F5DBFC5-DAB8-429a-9D71-63353E022903}"),
...
{
	RegDeleteKey(HKEY_CLASSES_ROOT, _T("CLSID\\{8F5DBFC5-DAB8-429a-9D71-63353E022903}\\InProcServer32"));
	RegDeleteKey(HKEY_CLASSES_ROOT, _T("CLSID\\{8F5DBFC5-DAB8-429a-9D71-63353E022903}"));
	return S_OK;
}