...
namespace RUGE
{
	...
	void CParticleSystem::Update(float fDelta)
	{
		...
		for (int i=0; i<m_nParticlesAlive; i++, pPar++)
		{
			...
			pPar->vecVelocity+=(vecAccel+vecAccel2)*fDelta;
			pPar->vecVelocity.y+=pPar->fGravity*fDelta;
			pPar->vecLocation+=pPar->vecVelocity*fDelta;
			pPar->fSpin+=pPar->fSpinDelta*fDelta;
			pPar->fSize+=pPar->fSizeDelta*fDelta;
			pPar->colColor+=pPar->colColorDelta*fDelta;
			if(m_bUpdateBoundingBox) m_rectBoundingBox.Encapsulate(pPar->vecLocation.x, pPar->vecLocation.y);
		}
		if (m_fAge != -2.0f)
		{
			float fParticlesNeeded=m_Info.nEmission*fDelta+m_fEmissionResidue;
	...
	void CParticleSystem::GetPosition(float *x, float *y) const
	{
		*x=m_vecLocation.x;
		*y=m_vecLocation.y;
	}
