...
void CSprite::Render(float x, float y, float fRot/* =0.0f */, float fHScale/* =1.0f */, float fVScale/* =0.0f */)
{
...
void CSprite::GetBoundingBox(float x, float y, CRect &rect) const
{
	rect.Set(x-m_fHotx, y-m_fHoty, x-m_fHotx+m_fWidth, y-m_fHoty+m_fHeight);
}

void CSprite::GetBoundingBox(float x, float y, float fRot, float fHScale, float fVScale, CRect &rect) const
{
	rect.Clear();
	if (fVScale==0) fVScale=fHScale;
	...
	if (fRot!=0.0f)
	{
		...
		rect.Encapsulate(tx1*fCost-ty1*fSint+x, tx1*fSint+ty1*fCost+y);
		rect.Encapsulate(tx2*fCost-ty1*fSint+x, tx2*fSint+ty1*fCost+y);
		rect.Encapsulate(tx2*fCost-ty2*fSint+x, tx2*fSint+ty2*fCost+y);
		rect.Encapsulate(tx1*fCost-ty2*fSint+x, tx1*fSint+ty2*fCost+y);
	}
	else
	{
